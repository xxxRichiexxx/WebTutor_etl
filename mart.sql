...
SELECT
	...
	COALESCE(
		LAST_VALUE(sq4."МП. Факт" ignore nulls) OVER (PARTITION BY s.subdivision_id  ORDER BY years."Год"),
		0) 																										AS "МП. Факт",
	st."Cтарший менеджер по продажам (СтМОП)" 																	AS "Cтарший менеджер по продажам (СтМОП). План",
	COALESCE(
		LAST_VALUE(sq8."СтМОП. Факт" ignore nulls) OVER (PARTITION BY s.subdivision_id  ORDER BY years."Год"),
		0) 																										AS "СтМОП. Факт",
	st."Эксперт по корпоративным продажам (ЭКорП)" 																AS "Эксперт по корпоративным продажам (ЭКорП). План",
	COALESCE(
		LAST_VALUE(sq12."ЭКорП. Факт" ignore nulls) OVER (PARTITION BY s.subdivision_id  ORDER BY years."Год"),
		0) 																										AS "ЭКорП. Факт",
	st."Руководитель отдела продаж (РОП)" 																		AS "Руководитель отдела продаж (РОП). План",
	COALESCE(
		LAST_VALUE(sq16."РОП. Факт" ignore nulls) OVER (PARTITION BY s.subdivision_id  ORDER BY years."Год"),
		0) 																										AS "РОП. Факт",
	COALESCE(
		LAST_VALUE(sq20."Маркетолог. Факт" ignore nulls) OVER (PARTITION BY s.subdivision_id  ORDER BY years."Год"),
