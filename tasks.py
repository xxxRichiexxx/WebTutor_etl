...
source_host = 'cl02sql\inst02sql'
source_db = 'datatutor'
eng_str = fr'mssql://{source_username}:{source_password}@{source_host}/{source_db}?driver=SQL Server'

source_engine =sa.create_engine(eng_str)


dwh_password = quote('s@vy7hSA')
dwh_engine = sa.create_engine(
    f'vertica+vertica_python://shveynikovab:{dwh_password}@vs-da-vertica:5433/sttgaz?'
)
